...
class Followers:
    ...
    def takeInput(self) -> str:
        ...
        while True:
            ...
            if cmd[0] == "select" and cmd[1].isnumeric() and len(cmd) == 2:
            ...
            elif cmd[0] == "scroll" and cmd[1] == "down" and len(cmd) == 2 and followerSelected: self.scrollDown()     
            elif cmd[0] == "scroll" and cmd[1] == "up" and len(cmd) == 2 and followerSelected: self.scrollUp()   
                ...
                else: ComposeTweet.createTweet(self.followerTweetList[int(cmd[1])-1][0])
            elif cmd[0] == "retweet" and cmd[1].isnumeric() and len(cmd) == 2 and followerSelected:
                if int(cmd[1]) > len(self.followerTweetList) or int(cmd[1]) < 1: print("Invalid tweet selected.")
                else: ComposeTweet.createRetweet(self.followerTweetList[int(cmd[1])-1][0])
    ...
    def getFollowerInfo(self, flwer:int) -> None:
        ...
        print("\nYou are looking at " + Follow.getName(self.follower) + "'s profile.")
        print("Tweet Count: {}\t Followers: {} \t Following: {}".format(tweets, followers, followees))
    ...
    def getNumberOfTweets(self) -> int:
        ...
        if tweetResult == None: tweetResult[0] = 0 
        if retweetResult == None: retweetResult[0] = 0
